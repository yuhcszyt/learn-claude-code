...
- **Current track: `claude-code/s01-s20`**
  The `s01_*` ... `s20_*` folders in this directory are the canonical version. Each chapter contains a full narrative README, translations, runnable `code.py`, and diagrams where needed.
- **Legacy transition track: `claude-code/docs/`, `claude-code/agents/`, and `claude-code/web/`**
  These preserve the older 12-lesson version and web presentation while the new 20-lesson track settles.

If you are starting now, read the `claude-code/s01_agent_loop/` through `claude-code/s20_comprehensive/` chapters. If you are following an older link or using the web app, be aware that legacy and current chapter numbers do not always match.
...
python claude-code/s01_agent_loop/code.py        # Start here -- one loop + bash
python s08_context_compact/code.py   # Context compaction (complex)
python claude-code/s20_comprehensive/code.py     # Endpoint: all mechanisms in one loop
```

### Legacy 12-Lesson Track

```sh
python claude-code/agents/s01_agent_loop.py
python claude-code/agents/s12_worktree_task_isolation.py
python claude-code/agents/s_full.py
```

### Web Platform

The web app renders course content from this `claude-code/` folder.

```sh
cd claude-code/web && npm install && npm run dev   # http://localhost:3000
```
...
```
learn-claude-code/
  claude-code/s01_agent_loop/          # one folder per chapter
    README.md              #   Chinese source (complete narrative)
    README.en.md           #   English translation
    README.ja.md           #   Japanese translation
    code.py                #   standalone runnable code
    images/                #   SVG diagrams
  claude-code/s02_tool_use/
  ...
  claude-code/s19_mcp_plugin/
  claude-code/s20_comprehensive/       # endpoint chapter
  claude-code/agents/                  # legacy 12 runnable copies + s_full.py
  claude-code/skills/                  # skill files used by s07
  claude-code/docs/                    # legacy 12-lesson docs, kept during transition
  claude-code/web/                     # course learning site
  claude-code/tests/
```
