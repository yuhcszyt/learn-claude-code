...
- **新版主线：`claude-code/s01-s20`**
  本目录下的 `s01_*` 到 `s20_*` 是新的主版本，也是当前推荐阅读路径。每章包含完整叙事 README、英文/日文译本、可运行的 `code.py`，以及必要的图示。
- **旧版过渡：`claude-code/docs/`、`claude-code/agents/` 和 `claude-code/web/`**
  这些仍保留旧 12 章体系和 Web 展示，暂时用于已有读者、旧链接和平台过渡。

新读者请从 `claude-code/s01_agent_loop/` 读到 `claude-code/s20_comprehensive/`。如果你是从旧链接或 Web 平台进入，注意旧版章节号和新版不完全一致，不要混用章节号。
...
python claude-code/s01_agent_loop/code.py        # 起点 — 一个循环 + bash
python s08_context_compact/code.py    # 上下文压缩（复杂章）
python claude-code/s20_comprehensive/code.py      # 终点章: 全部机制归到一个循环
```
...
```
learn-claude-code/
  claude-code/s01_agent_loop/          # 每章一个文件夹
  ...
  ...
  s19_mcp_plugin/
  claude-code/s20_comprehensive/       # 终点章
  claude-code/agents/                  # 旧 12 章可运行副本 + s_full.py
  skills/                  # s07 使用的 skill 文件
  docs/                    # 旧 12 章文档，过渡期保留
  claude-code/web/                     # 课程学习站点
  tests/
```
